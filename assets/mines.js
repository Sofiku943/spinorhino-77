...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-d90cac8e";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-d90cac8e">'+'<span class="tile-face-d90cac8e tile-front-d90cac8e"></span>'+'<span class="tile-face-d90cac8e tile-back-d90cac8e"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d90cac8e")){return;}
var back=tile.querySelector(".tile-back-d90cac8e");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-d90cac8e");if(isMine){tile.classList.add("boom-d90cac8e");}
if(dim){tile.classList.add("dimmed-d90cac8e");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-d90cac8e")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-d90cac8e");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d90cac8e")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-d90cac8e");void panelEl.offsetWidth;panelEl.classList.add("jolt-d90cac8e");}
endRound();}
